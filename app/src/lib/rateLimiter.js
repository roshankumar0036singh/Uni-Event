...
export async function enforceRateLimit(isEventCreation = false) {
    ...
    await runTransaction(db, async (transaction) => {
        ...
        let lastMillis = 0;
        if (lastWrite) {
            lastMillis = lastWrite.toDate ? lastWrite.toDate().getTime() : new Date(lastWrite).getTime();
        }
        ...
        if (isEventCreation) {
            const dailyCount = (userData.lastEventDay || 0) === currentDayInt ? (userData.eventCountDay || 0) : 0;
            if (dailyCount >= 5) {
