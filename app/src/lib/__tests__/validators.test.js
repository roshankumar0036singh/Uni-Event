...
describe('eventSchema', () => {
    ...
    it('rejects missing required fields', async () => {
        await expect(validate({}, eventSchema)).rejects.toThrow(
            /title|description|category|startAt/,
        );
    });

    it('rejects title shorter than 3 characters', async () => {
        await expect(validate({ ...validEvent, title: 'AB' }, eventSchema)).rejects.toThrow(
    ...
    it('rejects invalid price type', async () => {
        await expect(validate({ ...validEvent, price: 'free' }, eventSchema)).rejects.toThrow(
            'price',
        );
    });

    it('rejects negative price', async () => {
        await expect(validate({ ...validEvent, price: -10 }, eventSchema)).rejects.toThrow(
            'cannot be negative',
        );
    });
...
describe('eventUpdateSchema', () => {
    ...
    it('rejects invalid field type', async () => {
        await expect(validate({ price: 'not-a-number' }, eventUpdateSchema)).rejects.toThrow(
            'price',
        );
    });
});
...
describe('userSchema', () => {
    ...
    it('rejects invalid email', async () => {
        await expect(validate({ email: 'not-an-email' }, userSchema)).rejects.toThrow(
            'Invalid email',
        );
    });
...
describe('feedbackSchema', () => {
    ...
    it('rejects missing description', async () => {
        await expect(validate({ userId: 'u1' }, feedbackSchema)).rejects.toThrow(
...
describe('messageSchema', () => {
    it('accepts valid message', async () => {
        const result = await validate({ text: 'Hello everyone!', userId: 'u1' }, messageSchema);
        expect(result.text).toBe('Hello everyone!');
    });

    it('rejects empty message', async () => {
        await expect(validate({ text: '', userId: 'u1' }, messageSchema)).rejects.toThrow(
