...
export const generateBranchParticipationReport = functions.https.onCall(async (data, context) => {
    ...
    for (const eventDoc of eventsSnap.docs) {
        ...
        const branchCountEntries =
            branchCounts && typeof branchCounts === 'object' && !Array.isArray(branchCounts)
    ...
    const rows = Array.from(statsByBranch.values())
        ...
        .map((stats, index) => ({
            ...
            registrationShare:
                totalRegistrations > 0 ? (stats.registrations / totalRegistrations) * 100 : 0,
            attendanceRate:
                stats.registrations > 0 ? (stats.attendance / stats.registrations) * 100 : 0,
        }));
