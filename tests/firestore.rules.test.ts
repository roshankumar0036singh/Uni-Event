...
describe("Firestore Security Rules", () => {
  ...
  test("Unauthenticated user writes /events -> denied", async () => {
    const db = getFirestoreContext();
    await assertFails(
      setDoc(doc(db, "events/event1"), { title: "Hackathon" })
    );
  });
  ...
  test("Student tries to create event -> denied", async () => {
    ...
    await assertFails(
      setDoc(doc(db, "events/event1"), { title: "Unauthorized Event" })
    );
  });
  ...
  test("User creates reminder for another user -> denied", async () => {
    const db = getFirestoreContext("student1");
    await assertFails(
      setDoc(doc(db, "reminders/rem1"), { userId: "student2", text: "Unauthorized reminder" })
    );
  });
  ...
  test("Participant updates own record -> allowed", async () => {
    ...
    const db = getFirestoreContext("student1");
    await assertSucceeds(
      setDoc(doc(db, "events/event1/participants/student1"), { joined: false }, { merge: true })
    );
  });
  ...
  test("User creates own feedback -> allowed", async () => {
    const db = getFirestoreContext("student1");
    await assertSucceeds(
      setDoc(doc(db, "events/event1/feedback/student1"), { rating: 5 })
    );
  });
  ...
  test("User creates feedback for another user -> denied", async () => {
    const db = getFirestoreContext("student1");
    await assertFails(
      setDoc(doc(db, "events/event1/feedback/student2"), { rating: 5 })
    );
  });
  ...
  test("Authenticated user creates event message -> allowed", async () => {
    const db = getFirestoreContext("student1");
    await assertSucceeds(
      setDoc(doc(db, "events/event1/messages/msg1"), { text: "Hello" })
    );
  });
